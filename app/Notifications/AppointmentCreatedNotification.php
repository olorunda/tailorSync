<?php

namespace App\Notifications;

use App\Models\Appointment;
use App\Notifications\Channels\PushNotificationChannel;
use Illuminate\Bus\Queueable;
use Illuminate\Contracts\Queue\ShouldQueue;
use Illuminate\Notifications\Messages\MailMessage;
use Illuminate\Notifications\Notification;

class AppointmentCreatedNotification extends Notification implements ShouldQueue
{
    use Queueable;

    protected $appointment;

    /**
     * Create a new notification instance.
     */
    public function __construct(Appointment $appointment)
    {
        $this->appointment = $appointment;
    }

    /**
     * Get the notification's delivery channels.
     *
     * @return array<int, string>
     */
    public function via(object $notifiable): array
    {
        $channels = ['mail', 'database'];

        // Add push notification channel if the notifiable has push subscriptions
        if (method_exists($notifiable, 'pushSubscriptions') && $notifiable->pushSubscriptions()->exists()) {
            $channels[] = PushNotificationChannel::class;
        }

        return $channels;
    }

    /**
     * Get the mail representation of the notification.
     */
    public function toMail(object $notifiable): MailMessage
    {
        $formattedDate = $this->appointment->start_time->format('l, F j, Y');
        $formattedTime = $this->appointment->start_time->format('g:i A');
        $endTime = $this->appointment->end_time ? $this->appointment->end_time->format('g:i A') : null;

        $timeRange = $endTime ? "{$formattedTime} - {$endTime}" : $formattedTime;

        $clientInfo = '';
        if ($this->appointment->client) {
            $clientInfo = "Client: {$this->appointment->client->name}";
            if ($this->appointment->client->email) {
                $clientInfo .= " ({$this->appointment->client->email})";
            }
            if ($this->appointment->client->phone) {
                $clientInfo .= " - {$this->appointment->client->phone}";
            }
        }

        $orderInfo = '';
        if ($this->appointment->order) {
            $orderInfo = "This appointment is related to order #{$this->appointment->order->order_number}.";
        }

        return (new MailMessage)
            ->subject("New Appointment Created: {$this->appointment->title}")
            ->greeting("Hello {$notifiable->name},")
            ->line("A new appointment has been created in your calendar.")
            ->line("**Appointment Details:**")
            ->line("**Title:** {$this->appointment->title}")
            ->line("**Date:** {$formattedDate}")
            ->line("**Time:** {$timeRange}")
            ->line("**Location:** " . ($this->appointment->location ?: 'Our store'))
            ->when($this->appointment->description, function ($message) {
                return $message->line("**Description:** {$this->appointment->description}");
            })
            ->when($clientInfo, function ($message) use ($clientInfo) {
                return $message->line("**{$clientInfo}**");
            })
            ->when($orderInfo, function ($message) use ($orderInfo) {
                return $message->line($orderInfo);
            })
            ->action('View Appointment Details', url('/appointments/' . $this->appointment->id))
            ->line('Thank you for using our application!');
    }

    /**
     * Get the array representation of the notification.
     *
     * @return array<string, mixed>
     */
    public function toArray(object $notifiable): array
    {
        $formattedDate = $this->appointment->start_time->format('l, F j, Y');
        $formattedTime = $this->appointment->start_time->format('g:i A');
        $endTime = $this->appointment->end_time ? $this->appointment->end_time->format('g:i A') : null;

        $timeRange = $endTime ? "{$formattedTime} - {$endTime}" : $formattedTime;

        $clientInfo = '';
        if ($this->appointment->client) {
            $clientInfo = "Client: {$this->appointment->client->name}";
            if ($this->appointment->client->email) {
                $clientInfo .= " ({$this->appointment->client->email})";
            }
            if ($this->appointment->client->phone) {
                $clientInfo .= " - {$this->appointment->client->phone}";
            }
        }

        $orderInfo = '';
        if ($this->appointment->order) {
            $orderInfo = "This appointment is related to order #{$this->appointment->order->order_number}.";
        }

        // Prepare lines for the mail message
        $lines = [
            "A new appointment has been created in your calendar.",
            "**Appointment Details:**",
            "**Title:** {$this->appointment->title}",
            "**Date:** {$formattedDate}",
            "**Time:** {$timeRange}",
            "**Location:** " . ($this->appointment->location ?: 'Our store')
        ];

        // Add description if available
        if ($this->appointment->description) {
            $lines[] = "**Description:** {$this->appointment->description}";
        }

        // Add client info if available
        if ($clientInfo) {
            $lines[] = "**{$clientInfo}**";
        }

        // Add order info if available
        if ($orderInfo) {
            $lines[] = $orderInfo;
        }

        // Store the mail message components
        $mailData = [
            'subject' => "New Appointment Created: {$this->appointment->title}",
            'greeting' => "Hello {$notifiable->name},",
            'lines' => $lines,
            'action' => [
                'text' => 'View Appointment Details',
                'url' => url('/appointments/' . $this->appointment->id)
            ]
        ];

        return [
            'appointment_id' => $this->appointment->id,
            'title' => $this->appointment->title,
            'start_time' => $this->appointment->start_time->toIso8601String(),
            'end_time' => $this->appointment->end_time ? $this->appointment->end_time->toIso8601String() : null,
            'location' => $this->appointment->location,
            'order_id' => $this->appointment->order_id,
            'client_id' => $this->appointment->client_id,
            'mail' => $mailData
        ];
    }

    /**
     * Get the push notification representation of the notification.
     *
     * @param  mixed  $notifiable
     * @return array
     */
    public function toPushNotification($notifiable): array
    {
        $formattedDate = $this->appointment->start_time->format('M j');
        $formattedTime = $this->appointment->start_time->format('g:i A');

        return [
            'title' => "New Appointment",
            'body' => "A new appointment '{$this->appointment->title}' has been scheduled for {$formattedDate} at {$formattedTime}.",
            'icon' => '/apple-touch-icon.png',
            'badge' => '/apple-touch-icon.png',
            'tag' => 'appointment-created-' . $this->appointment->id,
            'url' => url('/appointments/' . $this->appointment->id),
            'data' => [
                'appointment_id' => $this->appointment->id,
                'type' => 'appointment_created'
            ]
        ];
    }
}
